...
class WriteService:
    def __init__(self):
        # PaddleOCR 로딩 (로그 없이 조용히 로딩)
        try:
            self.paddle = PaddleOCR(lang='korean', show_log=False)
        except:
            self.paddle = None
    ...
    def run_naver(self, file_bytes, filename):
        try:
            ...
            resp = requests.post(self.naver_url, headers=headers, data={'message': json.dumps(data)}, files=[('file', (filename, file_bytes))])
            
            texts = [f['inferText'] for img in resp.json().get('images', []) for f in img.get('fields', [])]
            return " ".join(texts)
        except:
            return "Naver OCR 실패"

    def run_paddle(self, file_bytes):
        if not self.paddle: return "Paddle 모델 없음"
    ...
    def run_check(self, text, question):
        try:
            res = ollama.chat(
                ...
                messages=[
                    {'role': 'system', 'content': '질문에 예/아니오로 답하고 이유를 설명해.'},
    ...
    async def process_immigration(self, file, mode="naver"):
        ...
        questions = ["양식이 맞나요?", "이름과 서명이 있나요?", "한글이나 영어로 적혔나요?"]
        validations = [{"question": q, "answer": self.run_check(text, q)} for q in questions]
