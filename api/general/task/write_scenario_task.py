...
# 쓰기 문제 생성 메서드
# =========================================================
def _process_single_question(source_data: WriteScenarioSource) -> Dict[str, Any]:
    dotenv.load_dotenv()
    ...
    prompt = f"""
        You are a Korean language tutor creating handwriting practice questions.
        ...
        Task:
        1. Extract ONLY the core keyword/phrase from the user's answer.
        2. Convert it to Korean as a SHORT word or phrase (maximum 5-7 characters).
        3. Use natural, appropriate Korean expressions.
        
        Examples:
        Q: "시내까지 어떤 교통수단을 이용하실 겁니까?" A: "I will use the train" → "기차"
        Q: "마스크는 어떻게 해야 합니까?" A: "Cover your face" → "안면 가림"
        Q: "술은 몇 병 소지하고 계십니까?" A: "One bottle" → "한 병"
        Q: "환전은 어디에서 하셨습니까?" A: "At the bank" → "은행"
        Q: "왜 그 직업을 좋아합니까?" A: "Because it makes me happy" → "행복해요"
        
        Rules:
        - Maximum 5-7 Korean characters
        - Focus on key noun/verb only
        - Do NOT create full sentences
        - Remove unnecessary particles (은/는/이/가) when possible
        - If the answer contains "...", replace with a plausible Korean word
...
def get_writing_questions(username: str, source_list: list[WriteScenarioSource]):
    ...
    try:
        ...
        for _source in source_list:
            processed_questions.append(_process_single_question(_source))
