...
# Routes
@router.post(
    "/post", response_model=InterviewResponse, status_code=status.HTTP_201_CREATED
)
...
def get_my_interviews(
    ...
    for _user_interview in _user_interview_list:
        user_interview_list.append(
            InterviewResponse.model_validate(
                _user_interview.interview, from_attributes=True
            )
        )
    ## transform Interview SQLModel Class To InterviewResponse BaseModel
    _all_interview_list = session.exec(select(Interview)).all()
    all_interview_list = [
        InterviewResponse.model_validate(item, from_attributes=True)
        for item in _all_interview_list
    ]
...
def sampling_interview_list(
    ...
    # target_number = 5
    all_sample_list = [
        item for item in all_interview_list if item not in user_interview_list
    ]
    target_list = []
    ## level 1 list
    sample_list = [
        item for item in all_sample_list if item.type_code == InterviewLevel.EASY
    ]
    ...
    if len(target_list) < max_number:
        ...
        sample_list = [
            item for item in all_sample_list if item.type_code == InterviewLevel.NORMAL
        ]
        sample_number = (
            sample_number if len(sample_list) >= sample_number else len(sample_list)
        )
    ...
    if len(target_list) < max_number:
        ...
        print(sample_number, max_number, len(target_list))
        sample_list = [
            item for item in all_sample_list if item.type_code == InterviewLevel.HARD
        ]
        sample_number = (
            sample_number if len(sample_list) >= sample_number else len(sample_list)
        )
...
@router.post(
    "/answer/post/{room_id}",
    response_model=list[UserInterviewResponse],
...
async def add_user_answer(
    ...
    for _answer in answers:
        ...
        _new_answer = UserInterview(
            user_id=_user.id, interview_id=_answer.interview_id, answer=_answer.answer
        )
...
async def gen_write_stage_to_redis(session: Session, room_id: int, current_user: User):
    try:
        ...
        questWriteInfo = QuestWriteInfo(
            index=1, difficulty=QuestLevel.EASY, room_id=room_id, question=[]
        )
        ...
        await store.save_ready_stage(
            StageType.WRITING, current_user.username, questWriteInfo.model_dump_json()
        )
        logger.info("****** generate write stage")
...
async def gen_speak_stage_to_redis(
    user_interview: list[UserInterviewResponse], room_id: int, current_user: User
):
    try:
        logger.info("****** generate speak stage")
        user_id = current_user.id
        interview_ids = [interview.interview_id for interview in user_interview]
        speaking_problem_json = SpeakingService().generate_speaking_problem(
            user_id, interview_ids
        )
        store = StateStore()
        ...
        questSpeakInfo = QuestSpeakInfo(
            index=1, difficulty=QuestLevel.EASY, room_id=room_id, audio=[]
        )
        ...
        await store.save_ready_stage(
            StageType.SPEAKING, current_user.username, questSpeakInfo.model_dump_json()
        )
        logger.info("****** generate speak stage")
    except Exception as e:
        logger.warning(e)
...
def _process_single_question(
    ...
    try:
        result_data["word_data"]["pronunciation"] = korean_to_english_pronunciation(
            kor_q
        )
    except NameError:
...
def get_writing_questions(session: Session, user_id: int) -> Dict[str, Any]:
    """
    쓰기 문제 생성 (동기 함수로 변환)
    """
    print(f"\n{'='*60}")
    ...
    try:
        ...
        for user_int, interview in results:
            processed_questions.append(_process_single_question(user_int, interview))
